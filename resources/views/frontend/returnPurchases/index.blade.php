        ...
        <div class="col-md-12">
            @can('return_purchase_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.return-purchases.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $returnPurchase->id }}">
                                        ...
                                        <td>
                                            @can('return_purchase_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.return-purchases.show', $returnPurchase->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('return_purchase_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.return-purchases.edit', $returnPurchase->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('return_purchase_delete')
                                                <form action="{{ route('frontend.return-purchases.destroy', $returnPurchase->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('return_purchase_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.return-purchases.massDestroy') }}",
