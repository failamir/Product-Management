...
@can('damage_purchase_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.damage-purchases.create') }}">
                        ...
                        <tr data-entry-id="{{ $damagePurchase->id }}">
                            ...
                            <td>
                                @can('damage_purchase_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.damage-purchases.show', $damagePurchase->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('damage_purchase_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.damage-purchases.edit', $damagePurchase->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('damage_purchase_delete')
                                    <form action="{{ route('admin.damage-purchases.destroy', $damagePurchase->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('damage_purchase_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.damage-purchases.massDestroy') }}",
