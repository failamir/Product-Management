...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.returnPurchase.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.return-purchases.update", [$returnPurchase->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="return_reason">{{ trans('cruds.returnPurchase.fields.return_reason') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('return_reason') ? 'is-invalid' : '' }}" name="return_reason" id="return_reason">{!! old('return_reason', $returnPurchase->return_reason) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('purchases') ? 'is-invalid' : '' }}" name="purchases_id" id="purchases_id" required>
                    @foreach($purchases as $id => $entry)
                        <option value="{{ $id }}" {{ (old('purchases_id') ? old('purchases_id') : $returnPurchase->purchases->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('return_date') ? 'is-invalid' : '' }}" type="text" name="return_date" id="return_date" value="{{ old('return_date', $returnPurchase->return_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('refund_amount') ? 'is-invalid' : '' }}" type="number" name="refund_amount" id="refund_amount" value="{{ old('refund_amount', $returnPurchase->refund_amount) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('return_note') ? 'is-invalid' : '' }}" name="return_note" id="return_note">{!! old('return_note', $returnPurchase->return_note) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\ReturnPurchase::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $returnPurchase->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.return-purchases.storeCKEditorImages') }}', true);
...
Dropzone.options.photoDropzone = {
    url: '{{ route('admin.return-purchases.storeMedia') }}',
    maxFilesize: 2, // MB
